...
def main() -> None:
    ...
    if np.any(~np.isfinite(ferr)) or np.any(ferr <= 0):
        ...
        print(
            "warning: flux_err had bad values -> replaced with global scatter estimate"
        )
    ...
    t0_est = float(
        trial_offsets[best_idx]
    )  # Make sure it's a float (for later json type-handling)
    ...
    if not os.path.exists("tess_params.json"):
        raise RuntimeError(
            "tess_params.json not found! Run your MCMC script to create it."
        )
    ...
    print(f"\nResidual stats:")
    print(
        f"Full data: mean = {np.mean(flux_no_transit - 1.0):.6e}, "
        f"std = {np.std(flux_no_transit - 1.0):.6e}"
    )
    print(
        f"Out-of-transit: mean = {np.mean(f_out - 1.0):.6e}, "
        f"std = {np.std(f_out - 1.0):.6e}"
    )
    print(
        f"In-transit: mean = {np.mean(f_in - 1.0):.6e}, "
        f"std = {np.std(f_in - 1.0):.6e}"
    )
    ...
    freq_full, power_full = ls_full.autopower(
    ...
    freq_out, power_out = ls_out.autopower(
        minimum_frequency=0.01,
        maximum_frequency=20.0,
        samples_per_peak=10,
    )
    ...
    freq_in, power_in = ls_in.autopower(
        minimum_frequency=0.01,
        maximum_frequency=20.0,
        samples_per_peak=10,
    )
